    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            */
            ...
            // Este usuario tendrá la propiedad Saldo.
            Usuario autenticado = getVerifidUser(u);
            
            if (autenticado != null) {
                my_session.setAttribute("user", autenticado);
                request.getRequestDispatcher("autenticado.jsp").forward(request, response);
            } else {
                my_session.setAttribute("user", null);
                my_session.setAttribute("msg", "Usuario Incorrecto!");
...
/*            if (verifyUser(u)) {
                ...
                HttpSession my_session = request.getSession();
                my_session.setAttribute("user", u);
                request.getRequestDispatcher("autenticado.jsp").forward(request, response);
            } else {
...
//            out.println("</form>");             
     ...
     */
    private Usuario getVerifidUser(Usuario u) {
